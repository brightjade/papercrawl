...
- `ppr/s2_client.py` -- Single point of contact with the Semantic Scholar Graph API. Owns pacing and 429/403 exponential backoff, shared by all three endpoints: `match_title` (one paper by title, heavily throttled to ~0.3 req/s), `get_batch` (up to 500 IDs per request -- `CorpusId:`, `DOI:`), and `bulk_search` (venue+year, 1000 papers per page). Returns raw API dicts. `ENRICHMENT_FIELDS` requests `authors` alongside the citation metadata, which `match_verdict` needs to break ties titles alone cannot settle.
- `ppr/matching.py` -- Decides whether a Semantic Scholar record is the paper we crawled. `title_key` canonicalises titles across sources (NFKC, LaTeX and unicode Greek onto one token, then alphanumerics only); `match_verdict` returns `matched` / `matched_fuzzy` / `reject`, using author-surname overlap to break ties that titles alone cannot. `/search/match` answers an unindexed paper with its nearest fuzzy neighbour rather than with nothing, so an unchecked result is how a paper ends up wearing a stranger's citations.
- `ppr/enrich.py` -- Per-conference enrichment. Picks the cheapest workable path: **refresh** (batch by `CorpusId` from the existing enriched file), **id-cold** (batch by `DOI` from the crawler's `link`, for DBLP-sourced venues), or **title-cold** (bulk prefetch by venue+year, then per-title matching for the misses). Every run re-attempts papers no earlier run could match -- Semantic Scholar indexes a venue months after we crawl it, so "not found" expires. Each refresh also re-verifies the binding against the returned title and authors (`ppr/matching.py`) and unbinds a record that turns out to describe a different paper, which returns it to the same retry queue. Two guards refuse to overwrite good data with bad: `check_guards` bounds how many *papers* a run may lose -- refusing when `papers.jsonl` is empty or under 50% of the enriched count; `check_enrichment_coverage` bounds how much *enrichment* a run may lose -- refusing when fewer than 50% of previously matched papers would keep a `CorpusId`, which the paper count alone cannot see, and which now also caps how much a single run's verification is allowed to unbind. Writes atomically via temp file + `os.replace`, sorted by citation count.
